...
type AdvisorsBoardProps = {
  advisors: AdvisorOption[];
};

type PanelState =
  | { open: false }
  | { open: true; mode: "create" | "edit"; initial: AdvisorOption | null };

export function AdvisorsBoard({ advisors }: AdvisorsBoardProps) {
      ...
      <section className="overflow-hidden rounded-xl border border-dba-border bg-dba-surface shadow-[0_1px_2px_rgba(51,53,54,0.03)]">
            ...
            <tbody>
              {advisors.length === 0 ? (
                <tr>
                  ...
                  >
                    Nog geen adviseurs. Voeg de eerste adviseur toe.
                  </td>
                </tr>
              ) : (
        ...
        <div className="border-t border-dba-border px-4 py-3">
          <p className="text-[13px] text-dba-muted">
            {advisors.length === 0
              ? "0 adviseurs"
              : `${advisors.length} adviseur${advisors.length === 1 ? "" : "s"}`}
          </p>
        </div>
      </section>
